...
object ScalaTIKZ extends AppCLI[Conf]("scalatikz") {
  ...
  opt[String]('o', "output")
    .valueName("<output path>".bold)
    .optional()
    .unbounded() // TODO remove underlined
    .action { (path, conf) =>
  ...
  opt[String]('F', "format")
    .valueName("<PDF|PNG|JPEG|TEX>".bold)
  ...
  opt[Compiler]("compiler")
    .valueName("<compiler>".bold)
    .optional()
    .unbounded()
    .action((c, conf) => conf.copy(compiler = c))
    .text(
      s"Change the underlying compiler (default is pdflatex)." +
  ...
  opt[Char]("delim")
    .valueName("<character>".bold)
    .optional()
    .unbounded()
    .action((d, conf) => conf.copy(delimiters = conf.delimiters :+ d))
    .text(
      "Specify the delimiter of the input csv data file (default is comma)." +
        "\n\tNote: ".green.bold + "In case you need to specify TAB as a delimiter, just type $'\\t'.\n"
    )

  opt[String]('x', "x-column")
    .valueName("<index or name>".bold)
    .optional()
    .unbounded()
    .action { (xColumn, conf) =>
    ...
    .text(
      "X column name or column index (default is the indexes of the y values)." +
        s"\n\t${"Note:".green.bold} Indices start at 0.\n"
    )

  opt[String]('y', "y-column")
    .valueName("<index or name>".bold)
    .required()
    .unbounded()
    .action { (yColumn, conf) =>
    ...
    .text(
      "Y column name or column index (required)." +
        s"\n\t${"Note:".green.bold} Indices start at 0.\n"
    )

  opt[String]("x-error-column")
    .abbr("ex")
    .valueName("<index or name>".bold)
    .optional()
    .unbounded()
    ...
    .text(
      "X error column index or column name (optional)." +
        s"\n\t${"Note:".green.bold} Indexes start from 0.\n"
    )

  opt[String]("y-error-column")
    .abbr("ey")
    .valueName("<index or name>".bold)
    .optional()
    .unbounded()
  ...
  opt[Color]('c', "color")
    ...
    .text(
      s"Line color. Available line colors: ${Color.values.mkString(", ")}" +
  ...
  opt[Color]("fill-color")
    ...
    .text(
      s"Line color. Available fill colors: ${Color.values.mkString(", ")}" +
        "\n\tNote: ".green.bold + "Colors can also be mixed by using the symbol '#'. " +
        s"For instance ${"red#20#green".underlined} defines a color that has 20% red and 80% green.\n"
    )
  ...
  opt[Color]("mark-stroke")
    ...
    .text(
      s"Mark stroke color. Available line colors: ${Color.values.mkString(", ")}" +
        "\n\tNote: ".green.bold + "Colors can also be mixed by using the symbol '#'. " +
        s"For instance ${"red#20#green".underlined} defines a color that has 20% red and 80% green.\n"
    )
  ...
  opt[Color]("mark-fill")
    ...
    .text(
      s"Mark fill color. Available line colors: ${Color.values.mkString(", ")}" +
        "\n\tNote: ".green.bold + "Colors can also be mixed by using the symbol '#'. " +
        s"For instance ${"red#20#green".underlined} defines a color that has 20% red and 80% green.\n"
    )

  opt[Double]("mark-size")
    .abbr("mz")
    .valueName("<double>".bold)
    .unbounded()
    .optional()
    .action { (size, conf) =>
  ...
  opt[LineStyle]("line-style")
    ...
    .text(
      s"Set line style (default is solid)." +
        s"\n\t${"Available line styles:".green.bold} ${LineStyle.values.mkString(", ")}\n"
    )

  opt[Pattern]('p', "pattern")
    .valueName("<pattern>".bold)
    .unbounded()
    .optional()
    .action { (pattern, conf) =>
  ...
  opt[Seq[Double]]("axis")
    .valueName("<double,double,double,double>".bold)
    .optional()
    .unbounded()
    .action((limits, conf) =>
      conf.copy(figure = conf.figure.havingLimits(limits.head, limits(1), limits(2), limits(3)))
    )
  ...
  opt[Seq[Double]]("x-lim")
    .valueName("<double,double>".bold)
    .optional()
    .unbounded()
  ...
  opt[AxisLinePos]("x-axis-position")
    .valueName("<position>".bold)
    .optional()
    .unbounded()
    .action((position, conf) => conf.copy(figure = conf.figure.havingXAxisLinePos(position)))
    .text(
      s"Set figure's X axis position (default is $BOX). " +
        s"\n\t${"Available axis positions:".green.bold} ${AxisLinePos.values.mkString(", ")}\n"
    )

  opt[AxisLinePos]("y-axis-position")
    .valueName("<position>".bold)
    .optional()
    .unbounded()
    .action((position, conf) => conf.copy(figure = conf.figure.havingYAxisLinePos(position)))
    .text(
      s"Set figure's Y axis position (default is $BOX). " +
        s"\n\t${"Available axis positions:".green.bold} ${AxisLinePos.values.mkString(", ")}\n"
    )

  opt[Seq[Int]]('r', "rotate-ticks")
    .valueName("<int,int>".bold)
    .optional()
    .unbounded()
  ...
  opt[LegendPos]("legend-pos")
    ...
    .text(
      s"Change legend panel position (default is outer north east)." +
  ...
  opt[FontSize]("legend-font-size")
    .abbr("lf")
    .valueName("<size>".bold)
    .optional()
    .unbounded()
    .action((size, conf) => conf.copy(figure = conf.figure.havingLegendFontSize(size)))
    .text(
      s"Set legend font size (default is $NORMAL). " +
        s"\n\t${"Available font sizes:".green.bold} ${FontSize.values.mkString(", ")}\n"
    )

  opt[Unit]("x-log-scale")
    .optional()
    .unbounded()
    .action((_, conf) => conf.copy(figure = conf.figure.havingLogXAxis))
    .text("Enables logarithmic X scale (default is linear).\n")

  opt[Unit]("y-log-scale")
    .optional()
    .unbounded()
  ...
  opt[ColorMap]("color-map")
    .abbr("cmap")
    .valueName("<color map>".bold)
    .optional()
    .unbounded()
  ...
  opt[Color]("background-color")
    ...
    .text(
      s"Background color. Available colors: ${Color.values.mkString(", ")}" +
        "\n\tNote: ".green.bold + "Colors can also be mixed by using the symbol '#'. " +
        s"For instance ${"red#20#green".underlined} defines a color that has 20% red and 80% green.\n"
    )
    ...
    case Some(conf) if conf.graphics.nonEmpty =>
      ...
      conf.graphics.zipWithIndex.foreach {
        case (graphic, i) =>
          ...
          val coordinates: Coordinates2D = (CSV.parseColumns(
            ...
            Seq(graphic.xColumn, graphic.yColumn).flatten: _*
          ): @unchecked) match {
          ...
          val error: Option[Coordinates2D] = (CSV.parseColumns(
            new File(conf.inputs.applyOrElse(i, i => notFound(conf.inputs)(i))),
            conf.delimiters.applyOrElse(i, (_: Int) => ','), // the default delimiter is the comma
            Seq(graphic.xErrorColumn, graphic.yErrorColumn).flatten: _*
          ): @unchecked) match {
          ...
          graphic.graph match {
            ...
            case SCATTER => resultedFigure = resultedFigure.scatter(
            ...
            case MESH => resultedFigure = resultedFigure.scatterMesh(
                marker = graphic.marker.getOrElse(CIRCLE),
                markSize = graphic.markSize
              )(coordinates)

            case STEM => resultedFigure = resultedFigure.stem(
                lineColor = graphic.lineColor.getOrElse(nextColor),
                marker = graphic.marker.getOrElse(NONE),
                markStrokeColor = graphic.markStrokeColor.getOrElse(graphic.lineColor.getOrElse(currentColor)),
                markFillColor = graphic.markFillColor.getOrElse(graphic.lineColor.getOrElse(currentColor)),
      ...
      result match {
        case Success(f) => logger.info(s"${conf.format} successfully saved in $f.")
