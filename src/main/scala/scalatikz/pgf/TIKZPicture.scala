...
trait TIKZPicture extends Logging {
  ...
  private def compilePDF(compiler: Compiler = PDF_LATEX): File = {
    ...
    using(new PrintStream(s"$path/pgf-pie.sty")) { outputStream =>
      ...
      if (styStream == null) s"cp ./src/main/resources/pgf-pie.sty $path" ! devNullLogger
   ...
   */
  final def saveAsPDF(path: String, compiler: Compiler = PDF_LATEX): Try[File] = moveFile(compilePDF(compiler), path)
