...
import scalatikz.pgf.plots.enums.LineType.{ SHARP, STEPS }
...
final class Figure private[plots] (
    ...
    axisType: AxisSystem) extends TIKZPicture with PGFPlot {
...
final class BipolarFigure private[pgf] (
    override val name: String,
    protected val graphics: Seq[Figure]) extends TIKZPicture {
...
final class FigureArray private[pgf] (
    override val name: String,
    protected val graphics: Seq[Figure],
...
object DoubleAxisFigure {

  def apply(name: String): BipolarFigure =
    new BipolarFigure(name, Seq(
      Figure("").havingXAxisLinePos(AxisLinePos.BOTTOM).havingYAxisLinePos(AxisLinePos.LEFT),
      Figure("").havingXAxisLinePos(AxisLinePos.TOP).havingYAxisLinePos(AxisLinePos.RIGHT)
    ))
