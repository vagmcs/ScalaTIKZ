...
class Figure private (
   ...
   */
  def havingLimits(xMin: Double, xMax: Double, yMin: Double, yMax: Double): Figure = new Figure(
    colorIterator,
    name,
    axis.copy(xMin = Some(xMin), xMax = Some(xMax), yMin = Some(yMin), yMax = Some(yMax)),
    axisType,
    graphics,
    secondary
  )
   ...
   */
  def customPlot(pgf: PGFPlot): Figure = new Figure(colorIterator, name, axis, axisType, pgf :: graphics, secondary)
