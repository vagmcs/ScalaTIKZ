...
object DataTypes {
  ...
  object Data2D {
    ...
    implicit def fromDoubleFunction[X: Numeric, Y: Numeric](xf: (Seq[X], Double => Y)): Data2D = xf match {
      case (seq, f) =>
        new Data2D { val coordinates: Coordinates2D = seq.map(x => x.toDouble -> f(x.toDouble).toDouble) }
    }
  }
  ...
  object Data3D {
    ...
    implicit def fromZ[Z: Numeric](z: Seq[Z]): Data3D = new Data3D {
      val coordinates: Coordinates3D =
        ((1 to z.length) zip z.map(_.toDouble)).map { case (idx, z) => (idx.toDouble, idx.toDouble, z) }
    }
    ...
    implicit def fromDoubleFunction[X: Numeric, Y: Numeric, Z: Numeric](
      xyf: (Seq[(X, Y)], (Double, Double) => Z)
    ): Data3D = xyf match {
      case (seq, f) => new Data3D {
          val coordinates: Coordinates3D =
            seq.map { case (x, y) => (x.toDouble, y.toDouble, f(x.toDouble, y.toDouble).toDouble) }
        }
    }
  }
}
