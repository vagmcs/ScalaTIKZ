...
object CSV {
  ...
  private def toInputStream(inputFile: File): Try[InputStream] = inputFile.getName match {
    ...
    case fileName if fileName.matches(".*[.]gz") => Try(new GZIPInputStream(new FileInputStream(inputFile)))

    case fileName if fileName.matches(".*[.]zip") => Try(new ZipInputStream(new FileInputStream(inputFile)))

    case fileName if fileName.matches(".*[.]csv") => Try(new FileInputStream(inputFile))
  ...
  def parseColumnsByIndex(
    ...
    val results = Try {
      columns.map(processor.getColumn).toList.map(seq => seq.asScala.toList.map(_.toDouble))
    } match {
      case Success(xs) => xs
      case Failure(ex) => return Failure(ex)
    }
  ...
  def parseColumns(
    ...
    if (!columns.forall(_ forall Character.isDigit)) settings.setHeaderExtractionEnabled(true)
    ...
    val results = Try {
      if (columns.forall(_ forall Character.isDigit))
        columns.map(_.toInt).map(processor.getColumn).toList.map(seq => seq.asScala.toList.map(_.toDouble))
      else columns.map(processor.getColumn).toList.map(seq => seq.asScala.toList.map(_.toDouble))
    } match {
      case Success(xs) => xs
      case Failure(ex) => return Failure(ex)
    }
